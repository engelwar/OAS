    ...
    <div class="col-8 tabla_cont">
      <div class="row mb-2">
        <div class="col-12 d-flex">
          <form action="{{route('ventamarcaalmacen.index')}}" method="GET">
            @csrf
            @if(!isset($sucur))
        ...
        <div class="modal-body" style="height:85vh;overflow:auto;">
          <div class="row">
            <ul class="nav nav-tabs" id="graf" role="tablist">
              <li class="nav-item" role="presentation">
                <button class="nav-link nav-link-sm active" id="primerosB-tab" data-bs-toggle="tab" data-bs-target="#primerosB" type="button" role="tab" aria-controls="primerosB" aria-selected="false">Barras</button>
              </li>
              @if(Auth::user()->tienePermiso(15,10))
              <li class="nav-item" role="presentation">
                <button class="nav-link disabled" id="paretoG-tab" data-bs-toggle="tab" data-bs-target="#paretoG" type="button" role="tab" aria-controls="paretoG" aria-selected="false">Pareto</button>
              </li>
              @endif
            </ul>
  ...
  $(document).ready(function() {
    ...
    function mixanalisys() {
      ...
      var createTable = function(estado, filtro, xmp, marc, client) {
        ...
        $.ajax({
          url: "{{route('ventamarcaalmacen.getdatos')}}",
          type: 'POST',
          data: {
            client,
            marc
        ...
        var table = $('#tableex').DataTable({
          ...
          ajax: {
            ...
            url: "{{route('ventamarcaalmacen.general')}}",
            type: "post",
            data: function(d) {
              d.usuarios = usuarios;
              d.fini = $('#fini').val();
            ...
            if ($('input#graficoxmarca').is(':checked')) {
              ...
              if (xmp == "xProducto" && tab.length > max) {
                for (let i = 0; i < max; i++) {
                  dataG.push({
                    name: tab[i].marca,
                    value: parseFloat(tab[i].sumtot.replace(/,/g, ''))
                  });
                }
              } else {
